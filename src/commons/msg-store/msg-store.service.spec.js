describe('msgStore', () => {
	...
	it(
		'returns all messages through all()',
		inject(msgStore => {
	...
	it(
		'has one default message with date',
		inject(msgStore => {
	...
	it(
		...
		inject(msgStore => {
			...
			expect(msgStore._messages[msgStore._messages.length - 1].text).toEqual('Hello, FooBar!');
			expect(msgStore._messages[msgStore._messages.length - 1].date).toEqual(jasmine.any(Date));
		})
	);
});
